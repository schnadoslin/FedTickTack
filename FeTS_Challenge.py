...
from my_code.sitebatch import get_current_round
def ranked_dice_train(collaborators,
                            db_iterator,
                            fl_round,
    ...
    """
    #'site1': {'train_dice_per_label_0': 0.9, 'train_dice_per_label_1': 0.85, 'train_dice_per_label_2': 0.8, 'train_dice_per_label_4': 0.75, 'loss': 0.1, 'q': 0.1},
     site_metrics = {}
     for c in collaborators:
         site_metrics[c] = {"round":-1}
     for record in db_iterator:
        tensor_name = record['tensor_name']
        if(tensor_name.startswith('train_dice_per_label')):
            site = record['tags'][0]
            if (site == 'metric'):
                continue
            if(site_metrics[site]["round"]<=record['round']):
                site_metrics[site]["round"] = record['round'] 
                site_metrics[site][tensor_name] = record['nparray']
     
     return get_current_round(fl_round,collaborators,site_metrics)    
...
# this is a new aggregation function

# TENSOR DB Beispiel:
# print(tensorDB)
#                 tensor_name                         origin round report       tags                                            nparray
#0           ins.conv0.weight  aggregator_plan.yaml_4df030c7     0  False   (model,)  [[[[[ 0.05839302  0.08648419 -0.0347912 ]\n [-...
#1             ins.conv0.bias  aggregator_plan.yaml_4df030c7     0  False   (model,)  [-0.0538391, 0.031509954, 0.02790047, 0.068226...
#2           ins.conv1.weight  aggregator_plan.yaml_4df030c7     0  False   (model,)  [[[[[ 0.01078855  0.012304   -0.00358212]\n [-...
#3             ins.conv1.bias  aggregator_plan.yaml_4df030c7     0  False   (model,)  [0.010735477, -0.023078648, 0.022669798, -0.01...
#4           ins.conv2.weight  aggregator_plan.yaml_4df030c7     0  False   (model,)  [[[[[-0.01732179 -0.02274445 -0.00250995]\n [ ...
#...                      ...                            ...   ...    ...        ...                                                ...
#1415              train_dice                     aggregator     0   True  (metric,)                                           0.312724
#1416  train_dice_per_label_0                     aggregator     0   True  (metric,)                                           0.883724
#1417  train_dice_per_label_1                     aggregator     0   True  (metric,)                                           0.016948
#1418  train_dice_per_label_2                     aggregator     0   True  (metric,)                                           0.078433
#1419  train_dice_per_label_4                     aggregator     0   True  (metric,)                                           0.271792

#or 

#     tensor_name      origin round report         tags     nparray
#406         loss  aggregator     0   True  (2, metric)  0.36960208
#742         loss  aggregator     0   True  (3, metric)         1.0
#1078        loss  aggregator     0   True  (1, metric)   0.5526694
#1414        loss  aggregator     0   True    (metric,)    0.628173

import pandas as pd

from my_code.ticktack import ticktack
import torch
from openfl.utilities import LocalTensor
state_dict_cache = {}

def ticktack_aggregation(local_tensors,
                       tensor_db,
                       tensor_name,
                       fl_round,
                       collaborators_chosen_each_round, # {round: list of collaborators} number no string
                       collaborator_times_per_round): 
    root_path = os.path.abspath(os.getcwd())
    if(tensor_name== "ins.conv0.weight"):
        for colab in collaborators_chosen_each_round[fl_round]:
            pd = tensor_db.search(fl_round=fl_round, tags=(f'{colab}','metric'))    
            pd.to_csv(f'{root_path}/{fl_round}_{colab}_tensor_db.csv')
    #Einmalige Berechnung des Rückgabenetzwerkes am Anfang der FL-Runde
    sd = None
    if(os.path.exists(f'{root_path}/state_dict_{fl_round}.h5')):
        if fl_round not in state_dict_cache:
            state_dict_cache[fl_round] = torch.load(f'{root_path}/state_dict_{fl_round}.h5')
        sd = state_dict_cache[fl_round]
    else:    
        sd = ticktack(local_tensors=local_tensors,tensor_db=tensor_db,fl_round=fl_round,collaborators_chosen_each_round=collaborators_chosen_each_round)
    
    agg = None
    # just pass the optimizer state_dict for fl_round==0
    if(tensor_name not in sd):
        agg = np.average([t.tensor for t in local_tensors],axis=0)
    else:
        agg = sd[tensor_name]
...
# istensor_momentum_buffer
aggregation_function = ticktack_aggregation 
choose_training_collaborators = ranked_dice_train
...
institution_split_csv_filename = 'ranked_small_split.csv'#'partitioning_2.csv'#'small_split.csv'#partitioning_2.csv'#'small_split.csv'#'partitioning_2.csv' #'small_split.csv'#'small_split.csv'#'partitioning_2.csv' #'small_split.csv'#partitioning_2.csv'#'small_split.csv'#

# change this to point to the parent directory of the data
brats_training_data_parent_dir = '/data/MICCAI_FeTS2022_TrainingData/'
...
save_checkpoints = True #'experiment_35' #True
...
#restore_from_checkpoint_folder = 'experiment_2'
restore_from_checkpoint_folder = None #'experiment_40' #None #'experiment_34'#None #'experiment_34'
...
data_path = '/data/MICCAI_FeTS2022_ValidationData'
